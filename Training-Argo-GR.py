...
# shuffle
data_train, data_num_train, label_train =     data_utils.grouped_shuffle([data_train, data_num_train, label_train])
...
with tf.name_scope('metrics'):
    ...
    t_1_per_class_acc_op, t_1_per_class_acc_update_op =         tf.metrics.mean_per_class_accuracy(labels_sampled, predictions, setting.num_class,
                                           weights=labels_weights_sampled)
...
root_folder = os.path.join(cwd, "outfolder_argo_gr2")
...
with tf.Session( config=tf.ConfigProto(gpu_options=gpu_options) ) as sess:
    ...
    for batch_idx_train in range(batch_num):
        ...
        ######################################################################
        # Validation
        if (batch_idx_train % step_val == 0 and (batch_idx_train != 0 or args.load_ckpt is not None))                 or batch_idx_train == batch_num - 1:
            filename_ckpt = os.path.join(folder_ckpt, 'iter')
            saver.save(sess, filename_ckpt, global_step=global_step)
            print('{}-Checkpoint saved to {}!'.format(datetime.now(), filename_ckpt))
