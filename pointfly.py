...
def plot_confusion_matrix(correct_labels, predict_labels, labels, title='Confusion matrix', tensor_name = 'MyFigure/image', normalize=True):
    ...
    '''
    correct_labels = np.concatenate(correct_labels, axis=0).flatten()
    predict_labels = np.concatenate(predict_labels, axis=0).flatten()
    ...
    classes = labels #[re.sub(r'([a-z](?=[A-Z])|[A-Z](?=[A-Z][a-z]))', r'\1 ', x) for x in labels]
    ...
    c = ax.set_xticklabels(classes, fontsize=6, rotation=-90,  ha='center')
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        ax.text(j, i, "{0:.2f}".format(cm[i, j]) if cm[i,j]!=0 else '.', horizontalalignment="center", fontsize=8, verticalalignment='center', color= "black")
...
def get_indices(batch_size, sample_num, point_num, pool_setting=None):
    ...
    if(len(indices) < batch_size):
        for i in range(batch_size-len(indices)):
...
def augment(points, xforms, range_=None):
    points_xformed = tf.matmul(points, xforms, name='points_xformed')
    if range_ is None:
        return points_xformed

    jitter_data = range_ * tf.random_normal(tf.shape(points_xformed), name='jitter_data')
    jitter_clipped = tf.clip_by_value(jitter_data, -5 * range_[0], 5 * range_[0], name='jitter_clipped')
